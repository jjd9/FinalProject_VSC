function [xdot, y] = bicycle_model_hydro(t,x)
...
Fxfl = (mu_slip(omega_fl,vxfl)*hydro*Nf- FR*m*g*tanh(vx/0.1)/4); % front-left traction force
Fxfr = (mu_slip(omega_fr,vxfr)*hydro*Nf- FR*m*g*tanh(vx/0.1)/4); % front-right traction force
Fxrl = (mu_slip(omega_rl,vxrl)*hydro*Nr - FR*m*g*tanh(vx/0.1)/4); % rear-left traction force
Fxrr = (mu_slip(omega_rr,vxrr)*hydro*Nr - FR*m*g*tanh(vx/0.1)/4); % rear-right traction force

%Effective Torque
Tfl = (-mu_slip(omega_fl,vxfl)*hydro*Nf*R_w - drivetrain_damp *omega_fl);
Tfr = (-mu_slip(omega_fr,vxfr)*hydro*Nf*R_w- drivetrain_damp *omega_fr);
Trl = (Td - mu_slip(omega_rl,vxrl)*hydro*Nr*R_w- drivetrain_damp *omega_rl);
Trr = (Td - mu_slip(omega_rr,vxrr)*hydro*Nr*R_w- drivetrain_damp *omega_rr);
