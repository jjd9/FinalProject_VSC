function [xdot, y] = bicycle_model_lanekeeper(t,x)
...
global L B R_w L1 L2 h Jrl drivetrain_damp define_lane t_disturb
global m g mw Iz Cf Cr steer_angle alpha_maxf alpha_maxr
global Fyfd V_x Power_rated Gratio
global mu ifl ifr irl irr v_min fr break_on fix start
...
upper_limit = define_lane(1);
lower_limit = define_lane(2);
...
if Yt >= upper_limit && fix == 0
    fix  = -1;    
    start = t;
    display('upper')
elseif Yt <= lower_limit && fix == 0
    fix = 1;
    start = t;
        display('lower')
end
...
STEER = steer_angle;
deltaf = 0;
if (t>=start + 1 & t<start + 4) deltaf = STEER*fix; end;
if (t>=start + 4 & t<start + 4.2) deltaf = 0; end;
if (t>=start + 4.5 & t<start + 7.2) deltaf = -STEER*fix; end;
